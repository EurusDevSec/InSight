...
- **Core idea:** Chụp ảnh món ăn → Ước lượng thể tích (Depth Anything V2) → Tính GL → RAG Agent tư vấn liều Insulin
  ...
  - **RAG Agent** (Java/LangChain4j) — Tra cứu hướng dẫn y khoa, tư vấn cá nhân hóa
  - **Mobile App** (Flutter) — Chụp ảnh, hiển thị kết quả, Panic Mode
- **Tech Stack:** Flutter (Mobile) + Java Spring Boot (Backend/RAG) + Python (Vision) + gRPC + Kafka + PostgreSQL + Milvus + Redis
...
### 3. Code Standards
- **Java:** Spring Boot 3.3, Java 21, Virtual Threads, LangChain4j
- **Python:** 3.11+, PyTorch, Depth Anything V2, TorchServe
